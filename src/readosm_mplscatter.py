...
##########################################################
def plot_count_of_pictures():
    import mpl_scatter_density
    from astropy.visualization import LogStretch
    from astropy.visualization.mpl_normalize import ImageNormalize

    #Got 17500 from the linear plot
    norm = ImageNormalize(vmin=0., vmax=17500, stretch=LogStretch()) 

    df = pd.read_csv('data/20180901_peds_manhattan_workdays_crs4326.csv')
    x = df['lon']
    y = df['lat']
    dpi = 150 # 72, 96,150
    ...
    axcb.update_ticks()
    fig.savefig(os.path.join(outdir, 'count.pdf'))
